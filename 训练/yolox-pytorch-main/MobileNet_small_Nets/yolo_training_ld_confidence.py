...
class YOLOLoss(nn.Module):
    ...
    def forward(self, inputs,teacher_inputs,labels,T_fpn_output, S_fpn_output):
        outputs = []
        x_shifts = []
        y_shifts = []
        expanded_strides = []
        ...
        return self.get_losses(x_shifts, y_shifts, expanded_strides, labels, torch.cat(outputs, 1),
                               ...
                               tea_cls_pres,stu_cls_pres,T_fpn_output, S_fpn_output)
    ...
    def get_ld_loss(self,regs_prob_pre_softmax,teacher_regs_prob_pre_softmax,VLR_masks,T = 15 ,reg_max = 14):
        ...
        """
        Students_VLR_and_Main = regs_prob_pre_softmax.reshape(-1, 4, reg_max + 1)[VLR_masks] # [b * all_anchor_points,4,reg_max]
        Teachers_VLR_and_Main = teacher_regs_prob_pre_softmax.reshape(-1,4,reg_max + 1)[VLR_masks]

        soft_loss = nn.KLDivLoss(reduction="batchmean")
        # distillation_loss = soft_loss(
        #     F.softmax((Students_VLR_and_Main / T).view(-1,reg_max+1),dim = 1), # [b * all_anchor_points * 4,regmax+1]
        #     F.softmax((Teachers_VLR_and_Main / T).view(-1,reg_max+1),dim = 1)
        # )
        distillation_loss = soft_loss(F.log_softmax((Students_VLR_and_Main / T).view(-1,4,reg_max+1),dim = 2),
                                     F.softmax((Teachers_VLR_and_Main / T).view(-1,4,reg_max+1),dim = 2))*(T*T)

        # kd_loss = F.kl_div(
        #     F.log_softmax(pred / T, dim=1), target, ).mean(1) * (
        #                   T * T)

        # distillation_loss = F.softmax(distillation_loss, dim=1)

        return distillation_loss.sum()
    ...
    def get_Prediction_guided_feature_imitation_loss(self,tea_cls_pres,stu_cls_pres,T_fpn_output,S_fpn_output):
        ...
        self.Pre_guided_imitation_loss3 = Prediction_guided_feature_imitation_loss(64, 320).cuda()

        self.Pre_guided_imitation_loss4 = Prediction_guided_feature_imitation_loss(128, 640).cuda()

        self.Pre_guided_imitation_loss5 = Prediction_guided_feature_imitation_loss(256, 1280).cuda()
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs, regs_prob,
                   regs_prob_pre_softmax,teacher_regs_prob_pre_softmax,teacher_confidence_all,tea_cls_pres,stu_cls_pres,T_fpn_output, S_fpn_output,reg_max=14):
        """
        输入:
        1、x_shifts, y_shifts,expanded_strides 三张特征图上所有网格点的x,y坐标以及对应的stride ，list: [[w1*h1],[w2*h2],[w3*h3]]
        ...
        ################################################################################################################
        loss_PGF = self.get_Prediction_guided_feature_imitation_loss(tea_cls_pres, stu_cls_pres,T_fpn_output, S_fpn_output)
        ...
        return loss / num_fg + 5 * loss_distillation + loss_mse * 0.5 + 160000 * loss_PGF
    ...
    def get_in_boxes_info(self, gt_bboxes_per_image, expanded_strides, x_shifts, y_shifts, total_num_anchors, num_gt,
        ...
        expanded_strides_per_image = expanded_strides[
            0]  # expanded_strides:[1,total_num_anchors],   expanded_strides_per_image :[total_num_anchors]
