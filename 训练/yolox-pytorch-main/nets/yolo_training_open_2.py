...
class YOLOLoss(nn.Module):
    def __init__(self, num_classes, fp16, strides=[8, 16, 32]):
        ...
        self.fp16 = fp16
        self.memory_bank = memory_bank(4, 0.1, 150)

    def forward(self, inputs,IC_outputs, oln_outputs, unknown_outputs ,center_outputs,epoch, labels=None,is_training=True):
        outputs = []
        x_shifts = []
        y_shifts = []
        expanded_strides = []
        ...
        return self.get_losses(x_shifts, y_shifts, expanded_strides, labels, torch.cat(outputs, 1), IC_outputs, oln_outputs, unknown_outputs,center_outputs,epoch,is_training)
    ...
    def get_IC_Loss(self,feats,ious,gt_cls_all,epoch):
#         return self.memory_bank.get_IC_loss(feats,gt_cls_all,ious,epoch)
        return self.memory_bank.Prototype_IC_loss(feats,gt_cls_all,ious,epoch)

    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs, IC_outputs, oln_outputs, unknown_outputs,center_outputs,epoch,is_training=True):
        # -----------------------------------------------#
        ...
        center_outputs = torch.cat(center_outputs, dim=1)  # [b,all_anchors,1]
...
#             loss_up_bg = self.UP_loss_bg5(cls_preds.view(-1, self.num_classes),unknown_outputs.view(-1,1),oln_preds_all.squeeze(),obj_preds.view(-1, 1),bg_masks)
            ...
            loss_up_bg = self.UP_loss_bg7(cls_preds.view(-1, self.num_classes),unknown_outputs.view(-1,1),oln_preds_all.squeeze(),obj_preds.view(-1, 1),unknown_bg_masks)
            loss_up_bg = loss_up_bg if not torch.isnan(loss_up_bg) else cls_preds.new_tensor(0.0)
            
            loss = loss / num_fg  + loss_ic +  0.5*loss_up_bg 
...
#         return loss + loss_oln 
    ...
    def UP_loss_bg6(self,classification,unknown_pred,oln_out,obj_pred,negative_indices):
        ...
        if a.shape != torch.Size([0]):
            loss_unknown = - targets * torch.log(a) - (1- targets) * torch.log(1-a)
...
#         print(targets)
        ...
        if a.shape != torch.Size([0]):
            loss_unknown = (- targets * torch.log(a)).sum() / a.shape[0] 
