...
class YOLOXHead(nn.Module):
    def __init__(self, num_classes, width = 1.0, in_channels = [256, 512, 1024], act = "silu", depthwise = False,reg_max = 16):
        super().__init__()
        ...
        for i in range(len(in_channels)):
            ...
            self.reg_preds.append(
                nn.Conv2d(in_channels = int(256 * width), out_channels = 4*(reg_max+1), kernel_size = 1, stride = 1, padding = 0)
            )
            #####定义一个积分层################
    ...
    def forward(self, inputs):
        ...
        for k, x in enumerate(inputs):
            ...
            # reg_output  = self.Integral(reg_output)
            #---------------------------------------------------#
            #   判断特征点是否有对应的物体
            #   obj_pred 80, 80, 1
            #   obj_pred 40, 40, 1
...
class YoloBody_yuanshi_dfl(nn.Module):
    def __init__(self, num_classes, phi):
        ...
        # self.scales = nn.ModuleList([Scale(init_value=1.0) for _ in range(3)]).to(device=device)
    ...
    def forward(self, x):
        ...
        # fpn_outs,backbone_outs   = self.backbone.forward(x)
        outputs     = self.head.forward(fpn_outs)
        # print(outputs.shape)
        # for k,output in enumerate(outputs):
        #     output[..., :4] = self.scales[k](output[..., :4].clone())
