...
class YOLOLoss(nn.Module):
    def __init__(self, num_classes, fp16, strides=[8, 16, 32]):
        ...
        self.fp16 = fp16
        self.memory_bank = memory_bank(5, 0.1, 150)

    def forward(self, inputs,IC_outputs, oln_outputs, unknown_outputs,centerness ,epoch, labels=None,is_training = True):
        outputs = []
        x_shifts = []
        y_shifts = []
        expanded_strides = []
        ...
        return self.get_losses(x_shifts, y_shifts, expanded_strides, labels, torch.cat(outputs, 1), IC_outputs, oln_outputs, unknown_outputs,centerness,epoch,is_training)
...
#         xmin = output[...,0] - output[...,2] / 2
#         xmax = output[...,0] + output[...,2] / 2
#         ymin = output[...,1] - output[...,3] / 2
#         ymax = output[...,1] + output[...,3] / 2

#         grid = grid.type_as(output)
...
#         l_r = torch.cat([l,r],dim=2) # all_preds,2
#         t_b = torch.cat([t,b],dim=2)
#         p1,_ = torch.min(l_r,dim=2)
#         p1 = torch.clamp(p1, min=0.0001)
#         p2,_ = torch.max(l_r,dim=2)
#         p2 = torch.clamp(p2, min=0.0001)

#         p3,_ = torch.min(t_b,dim=2)
#         p3 = torch.clamp(p3, min=0.0001)
#         p4,_ = torch.max(t_b,dim=2)
#         p4 = torch.clamp(p4, min=0.0001)
  
#         centerness_target = torch.sqrt((p1/p2)*(p3/p4))
...
#         return self.memory_bank.get_IC_loss(feats,gt_cls_all,ious,epoch)
    ...
    def cal_centerness(self,cxs, cys,Center_target):
        # 真实框 左上角和右下角坐标
        xmin = Center_target[...,0] - Center_target[...,2] / 2
        xmax = Center_target[...,0] + Center_target[...,2] / 2
        ymin = Center_target[...,1] - Center_target[...,3] / 2
        ymax = Center_target[...,1] + Center_target[...,3] / 2
        ...
        ymax = ymax.squeeze()
        # 网格中心离真实框上下左右四条边的距离
        l = torch.abs(cxs - xmin).unsqueeze(dim=1)
        t = torch.abs(cys - ymin).unsqueeze(dim=1)
        r = torch.abs(xmax - cxs).unsqueeze(dim=1)
        b = torch.abs(ymax - cys).unsqueeze(dim=1)
        
        l_r = torch.cat([l,r],dim=1) # all_preds,2
        t_b = torch.cat([t,b],dim=1)
       
        p1,_ = torch.min(l_r,dim=1)
        p1 = torch.clamp(p1, min=0.0001)
        p2,_ = torch.max(l_r,dim=1)
        p2 = torch.clamp(p2, min=0.0001)
        
        p3,_ = torch.min(t_b,dim=1)
        p3 = torch.clamp(p3, min=0.0001)
        p4,_ = torch.max(t_b,dim=1)
        p4 = torch.clamp(p4, min=0.0001)
        
        centerness_target = torch.sqrt((p1/p2)*(p3/p4))
        return centerness_target

    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs, IC_outputs, oln_outputs, unknown_outputs,centerness,epoch,is_training = True):
        # -----------------------------------------------#
        ...
        centerness_outputs = torch.cat(centerness,dim=1)
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                gt_matched_classes, fg_mask, pred_ious_this_matching, matched_gt_inds, num_fg_img,x_centers,y_centers = self.get_assignments(
                ...
                unknown_fg_mask = (iou_all_target >= 0.6).bool() # 如果预测框和真实框的IoU 大于等于0.5,则视为前景
                unknown_bg_mask = (iou_all_target <= 0.2).bool()
                ...
                centerness_pred = centerness_outputs[batch_idx] 
                centerness_target = gt_bboxes_per_image[iou_max_idx,:]  # num_anchors,4
                
                center_preds.append(centerness_pred)
               
                center_targets.append(centerness_target)
                cxs.append(x_centers)
                cys.append(y_centers)
        ...
        loss_center = cls_preds.new_tensor(0.0)
        
        ########################################
        if len(oln_targets) != 0:
            oln_targets = torch.cat(oln_targets, 0)
            oln_preds = torch.cat(oln_preds, 0)
            center_preds = torch.cat(center_preds,0)
            center_targets = torch.cat(center_targets,0) 
            
            cxs = torch.cat(cxs,0).reshape([-1]) # Num_all_anchor
            cys = torch.cat(cys,0).reshape([-1])
            ...
            Center_pred = center_preds[idx,:]
            Center_target = center_targets[idx].unsqueeze(dim=1) # pos,4
            cxs = cxs[idx] # pos
            cys = cys[idx] # pos
            
            Center_target = self.cal_centerness(cxs, cys,Center_target)
            ...
            if S != 0:
                ...
                Center_target = torch.index_select(Center_target, 0, index).to(OLN_pred.device)
      
                Center_pred = torch.index_select(Center_pred, 0, index)
              

                loss_oln = F.l1_loss(OLN_pred, OLN_target, reduction='mean')
             
                loss_center = F.l1_loss(Center_pred.squeeze(),Center_target,reduction='mean')
...
#         loss = loss if not torch.isnan(loss) else cls_preds.new_tensor(0.0)
        ...
        return loss + loss_oln + loss_center
...
#         OLN_pred = torch.index_select(OLN_pred, 0, index)
    ...
    def get_up_loss2(self, pos_cls, pos_unknown_pred,iou):
        ...
        max_entropy_indices = max_entropy_indices[:15]  # [10]
...
#         target = target * (1- target)
    ...
    def UP_loss_bg5(self,classification,unknown_pred,oln_out,obj_pred,negative_indices):
        ...
        idx = oln_pred >= 0.8
        val = torch.sum(torch.sigmoid(neg_cls) * torch.sigmoid(neg_obj),dim=1)
...
#         loss = - target * torch.log(pred)
    ...
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image,
        ...
        # -------------------------------------------------------#
        fg_mask, is_in_boxes_and_center,x_centers,y_centers = self.get_in_boxes_info(gt_bboxes_per_image, expanded_strides, x_shifts,
        ...
        return gt_matched_classes, fg_mask, pred_ious_this_matching, matched_gt_inds, num_fg,x_centers,y_centers 
    ...
    def get_in_boxes_info(self, gt_bboxes_per_image, expanded_strides, x_shifts, y_shifts, total_num_anchors, num_gt,
        ...
        return is_in_boxes_anchor, is_in_boxes_and_center,x_centers,y_centers
