...
def get_res(imgs_path,xmls_path,predictions_out_path,num_classes,iou_thresh = 0.3):
    ...
    for i in range(len(labels)):
        ...
        for j in range(length):
            ...
            if val0[j] < iou_thresh:
                ...
                cv2.putText(labelled, str(pred_per_image_cls[j]),  (int(pred_per_image_box[j][0]) , int(pred_per_image_box[j][1]) - 12), cv2.FONT_HERSHEY_SIMPLEX,1, (0,0,255),
                                2)
            ...
            # 类比相同时，为gt
                ...
                cv2.putText(labelled, str(pred_per_image_cls[j]), (int(pred_per_image_box[j][0]) , int(pred_per_image_box[j][1]) - 12), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0),
                                2)
                ...
                cv2.putText(labelled, str(pred_per_image_cls[j]), (int(pred_per_image_box[j][0]) , int(pred_per_image_box[j][1]) - 12), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255),
                            2)
    ...
    tp_cls = [] # 每一类000000000000000000000 的tp数
    pred_cls = [] # 每一类的预测数
    gt_cls = [] # 每一类的gt数
    tp_sum = 0
    pred_sum = 0
    ...
    for i in range(len(classes)):
        ...
        f.write("\n")
        tp_sum += tp_cnt[i]
        pred_sum +=  pred_cls_cnt[i]

        shibiezhunquelv.append(tp_cnt[i] / (tp_no_cls[i]*1.0))
        print("%s: 类无关的检测数量=%d, TP=%d,pred= %d,GT=%d;虚警=%d;漏警=%d" % (classes[i],tp_no_cls[i],
                                                               tp_cnt[i], pred_cls_cnt[i],gt_cls_cnt[i],xujing_cnt[i],gt_cls_cnt[i] - tp_no_cls[i]))
    ...
    for i in range(len(classes)):
        ...
        print("%s: 识别准确率=%s" % (classes[i],str(shibiezhunquelv[i])))

    print("平均: precision=%s,recall=%s,f1-score=%s,shibie = %s" %
              (
               str(precision_sum / 6),
               str(recall_sum / 6),
               str(f1_sum / 6),
