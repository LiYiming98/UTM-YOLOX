...
class YOLOLoss(nn.Module):
    ...
    def forward(self, inputs,teacher_inputs,labels):
        ...
        for k, (stride, output,teacher_output) in enumerate(zip(self.strides, inputs,teacher_inputs)):
            ...
            output, grid, reg_prob, student_reg_out_pre_softmax,teacher_reg_out_pre_softmax,teacher_confidence = self.get_output_and_grid(output, teacher_output,
                                                                                                k, stride)
            # grid:[1,h*w,2]
    ...
    def get_ld_loss(self,regs_prob_pre_softmax,teacher_regs_prob_pre_softmax,VLR_masks,T = 15 ,reg_max = 14):
        ...
        distillation_loss = F.kl_div(F.log_softmax(Students_VLR_and_Main / T,dim=1),
                                     F.softmax(Teachers_VLR_and_Main / T,dim=1),reduction='none').mean() * (T * T)
    ...
    def get_ld_loss_JS_Div(self,regs_prob_pre_softmax,teacher_regs_prob_pre_softmax,VLR_masks,T = 15 ,reg_max = 14):
        ...
        distillation_loss = (KLDivloss(log_mean_output,p_output) + KLDivloss(log_mean_output,q_output)) / 2
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs, regs_prob,
        ...
        loss_distillation = self.get_ld_loss_JS_Div(regs_prob_pre_softmax,teacher_regs_prob_pre_softmax,VLR_masks)
...
#         return loss / num_fg  
    ...
    def get_in_boxes_info(self, gt_bboxes_per_image, expanded_strides, x_shifts, y_shifts, total_num_anchors, num_gt,
        ...
        # VLR_mask = is_in_boxes_and_center.sum(dim=0) > 0
