...
class YOLOLoss(nn.Module):
    ...
    def forward(self, inputs,IC_outputs, oln_outputs, unknown_outputs,centerness ,epoch, labels=None):
        outputs = []
        x_shifts = []
        y_shifts = []
        expanded_strides = []
        ...
        for k, (stride, output) in enumerate(zip(self.strides, inputs)):
            output, grid,centerness_target = self.get_output_and_grid(output, k, stride)
            x_shifts.append(grid[:, :, 0])
            y_shifts.append(grid[:, :, 1])
            expanded_strides.append(torch.ones_like(grid[:, :, 0]) * stride)
            outputs.append(output)

        return self.get_losses(x_shifts, y_shifts, expanded_strides, labels, torch.cat(outputs, 1), IC_outputs, oln_outputs, unknown_outputs,centerness,centerness_target,epoch)
    ...
    def get_output_and_grid(self, output, k, stride):
        ...
        p1 = torch.clamp(p1, min=0)
        p2,_ = torch.max(l_r,dim=2)
        p2 = torch.clamp(p2, min=1)

        p3,_ = torch.min(t_b,dim=2)
        p3 = torch.clamp(p3, min=0)
        p4,_ = torch.max(t_b,dim=2)
        p4 = torch.clamp(p2, min=1)

        centerness_target = torch.sqrt(p1/p2*p3/p4)


        return output, grid,centerness_target

    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs, IC_outputs, oln_outputs, unknown_outputs,centerness,centerness_targets,epoch):
        # -----------------------------------------------#
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                centerness_target = centerness_targets[batch_idx]
        ...
        #             loss_ic = self.get_IC_Loss(ic_needed_feats,ic_needed_ious,ic_needed_cls,epoch)
        ...
        if len(oln_targets) != 0:
            ...
            Center_target = center_targets[idx].unsqueeze(dim=1)
            ...
            loss_center = F.l1_loss(Center_pred,Center_target,reduction='mean')
        ...
        if epoch >= 60:
            loss_up = self.get_up_loss(cls_targets, cls_preds.view(-1, self.num_classes)[fg_masks],
                                       unknown_outputs.view(-1,1)[fg_masks])
            loss = loss / num_fg + loss_up
        else:
            loss = loss / num_fg
        ...
        return loss + loss_oln



    def get_up_loss(self, pos_target, pos_cls, pos_unknown_pred):
        """
        ...
        # 正样本 最大类别概率和对应的标签
        val, labels = torch.max(pos_target[hard_example_indice], dim=1)
        num_sample, num_classes = scores.shape

        mask = torch.arange(num_classes).repeat(num_sample, 1).to(pos_target.device)

        inds = mask != labels[:, None].repeat(1, num_classes)
        # hard_num,num_classes
        mask = mask[inds].reshape(num_sample, num_classes - 1)

        # gt 的概率 [pos_num,1]
        gt_scores = torch.gather(
            F.softmax(pos_cls, dim=1), 1, labels[:, None]
        ).squeeze(1)

        # 非gt的概率 ： [pos_num,num_classes - 1]
        mask_scores = torch.gather(pos_cls, 1, mask)

        gt_scores[gt_scores < 0] = 0.0
        # 非gt的概率  + 未知类概率   ： [pos_num,num_classes - 1]
        # mask_scores = torch.cat([mask_scores,pos_unknown_pred[hard_example_indice]],dim=1)

        targets = torch.zeros_like(mask_scores)
        # 未知类概率的回归目标，不确定度  = gt_score * (1 - gt_score)
        #         targets[:,-1] = gt_scores * (1 - gt_scores).pow(1)
        targets[:, -1] = gt_scores * (1 - gt_scores).pow(1)
        # logprob = F.log_softmax(mask_scores,dim = 1)

        pos_unknown_pred = torch.sigmoid(pos_unknown_pred)
        pred = torch.clamp(pos_unknown_pred[hard_example_indice].squeeze(dim=1), min=1e-7, max=1 - 1e-7)

        #         print(pred)
        #         print(targets[:,-1])
        loss = - targets[:, -1] * torch.log(pred) - (1 - targets[:, -1]) * torch.log(1 - pred)
