...
class YOLOLoss(nn.Module):
    ...
    def forward(self, inputs, teacher_inputs, labels):
        outputs = []
        x_shifts = []
        y_shifts = []
        expanded_strides = []
        ...
        for k, (stride, output, teacher_output) in enumerate(zip(self.strides, inputs, teacher_inputs)):
            # 共在三个不同大小的特征图上进行目标检测
            # output, grid, reg_prob,teacher_output, teacher_reg_prob = self.get_output_and_grid(output,teacher_output, k, stride)
            output, grid, reg_prob, student_reg_out_pre_softmax, teacher_reg_out_pre_softmax, teacher_confidence = self.get_output_and_grid(
    ...
    def get_output_and_grid(self, output, teacher_output, k, stride, reg_max=14):
        ...
        teacher_reg_out_pre_softmax = teacher_output[...,
                                      :(reg_max + 1) * 4]  # 获得未经过softmax的回归值 [b,w*h,(reg_max + 1)*4]
        ...
        return output, grid, reg_out_pre_jifen, student_reg_out_pre_softmax, teacher_reg_out_pre_softmax, tercher_confidence
        ...
        for batch_idx in range(bbox_preds.shape[0]):  # 遍历一个批次中所有图片的结果
            ...
            if (num_gt == 0):
                ...
                bboxes_preds_per_image = bbox_preds[
                    batch_idx]  # bboxes_preds_per_image: [all_pred_bboxes, [cx, cy, w, h]] 每张图片所有预测框对应的四个回归值 [cx,cy,w,h]
                pair_wise_ious = self.bboxes_diou(gt_bboxes_per_image, bboxes_preds_per_image,
                                                  False)  # [num_gt,all_anchor_points],即计算预测框和所有gt的IOU
                is_in_VLR_and_main = torch.gt(pair_wise_ious,
                                              p_pos * p_vlr)  # [num_gt, total_num_anchors] 在预测框和所有gt的IOU矩阵中挑选出比p_pos * p_vlr大的位置
                is_in_VLR_and_main_all = is_in_VLR_and_main.sum(
                    dim=0) > 0  # [total_num_anchors] 按维度0求和，然后根据和是否>0来判断该预测框是否与某个gt的IOU>p_pos * p_vlr
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs, regs_prob,
                   regs_prob_pre_softmax, teacher_regs_prob_pre_softmax, teacher_confidence_all, reg_max=14):
        """
        输入:
        1、x_shifts, y_shifts,expanded_strides 三张特征图上所有网格点的x,y坐标以及对应的stride ，list: [[w1*h1],[w2*h2],[w3*h3]]
...
#         VLR_masks = self.get_VLR_mask(labels, outputs)  # [total_num_anchors * batch_size]
        ...
        print(sum(VLR))
        print(sum(back_ground))
        loss_distillation1 = self.get_ld_loss(regs_prob_pre_softmax, teacher_regs_prob_pre_softmax, VLR) # 蒸馏前景
        loss_distillation2 = self.get_ld_loss(regs_prob_pre_softmax, teacher_regs_prob_pre_softmax, back_ground) # 蒸馏背景
        loss_distillation = 4 * loss_distillation1 + loss_distillation2
        print(loss_distillation1)
        print(loss_distillation2)
        ################################################################################################################
...
#         loss_mse = self.get_confidence_feature_imitation(obj_preds, teacher_confidence_all)
        ...
        return loss / num_fg + loss_distillation

    #         return loss / num_fg

    def get_masked_fd_loss(self, S_fpn_output, T_fpn_output):

        self.Masked_fd_loss_feature3 = Masked_FeatureLoss(40, 320).cuda()

        self.Masked_fd_loss_feature4 = Masked_FeatureLoss(112, 640).cuda()
        self.Masked_fd_loss_feature5 = Masked_FeatureLoss(160, 1280).cuda()
        loss_mfd3 = self.Masked_fd_loss_feature3(S_fpn_output[0], T_fpn_output[0])
        loss_mfd4 = self.Masked_fd_loss_feature4(S_fpn_output[1], T_fpn_output[1])
        loss_mfd5 = self.Masked_fd_loss_feature5(S_fpn_output[2], T_fpn_output[2])
        loss_mfd = (loss_mfd3 + loss_mfd4 + loss_mfd5) / 3;
    ...
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image,
        ...
        VLR_mask,_ = self.get_in_boxes_info(gt_bboxes_per_image, expanded_strides, x_shifts,
                                                                 y_shifts, total_num_anchors, num_gt,center_radius=2)
        # -------------------------------------------------------#
        #   fg_mask                 [total_num_anchors]
    ...
    def bboxes_diou(self, bboxes_a, bboxes_b, eps=1e-7):
        ...
        rho2 = (bboxes_a[:, None, 0] - bboxes_b[:, 0]) ** 2 + (
                    bboxes_a[:, None, 1] - bboxes_b[:, 1]) ** 2  # (cx1-cx2)^2 + (cy1-cy2)^2 即 gt 和 pred的中心点距离
