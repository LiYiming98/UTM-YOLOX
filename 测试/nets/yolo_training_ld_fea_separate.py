...
from feature_imitation.Feature_separate import Feature_separate_Loss
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, inputs,teacher_inputs,labels,T_fpn_output,S_fpn_output):
        ...
        for k, (stride, output,teacher_output) in enumerate(zip(self.strides, inputs,teacher_inputs)):
            ...
            output, grid, reg_prob, student_reg_out_pre_softmax,teacher_reg_out_pre_softmax = self.get_output_and_grid(output, teacher_output,
                                                                                                k, stride)
            # grid:[1,h*w,2]
        ...
        return self.get_losses(x_shifts, y_shifts, expanded_strides, labels, torch.cat(outputs, 1),
                               torch.cat(regs_prob, 1),torch.cat(regs_prob_pre_softmax,1),torch.cat(teacher_regs_prob_pre_softmax,1),T_fpn_output,S_fpn_output)
    ...
    def get_output_and_grid(self, output,teacher_output, k, stride, reg_max=14):
        ...
        return output, grid, reg_out_pre_jifen,student_reg_out_pre_softmax,teacher_reg_out_pre_softmax
    ...
    def get_VLR_mask(self, labels, outputs,p_pos=0.5, p_vlr=0.6):
        ...
        return torch.cat(VLR_masks, 0)  # [total_num_anchors * batch_size]
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs, regs_prob, regs_prob_pre_softmax,teacher_regs_prob_pre_softmax,T_fpn_output,S_fpn_output,reg_max=14):
        """
        输入:
        1、x_shifts, y_shifts,expanded_strides 三张特征图上所有网格点的x,y坐标以及对应的stride ，list: [[w1*h1],[w2*h2],[w3*h3]]
        ...
        loss_mfd = self.get_masked_fd_loss(S_fpn_output,T_fpn_output)
        ...
        # return loss / num_fg + 5 * loss_distillation + 4 * loss_mfd

    def get_masked_fd_loss(self,S_fpn_output,T_fpn_output):

        self.Masked_fd_loss_feature3 = Masked_FeatureLoss(64, 320).cuda()

        self.Masked_fd_loss_feature4 = Masked_FeatureLoss(128, 640).cuda()
        self.Masked_fd_loss_feature5 = Masked_FeatureLoss(256, 1280).cuda()
    ...
    # def get_feature_separate_loss(self,S_fpn_output,T_fpn_output):
    #
    #     self.feature_separate_loss_feature3 = Feature_separate_Loss(64, 320).cuda()
    #
    #     self.Masked_fd_loss_feature4 = Feature_separate_Loss(128, 640).cuda()
    #     self.Masked_fd_loss_feature5 = Feature_separate_Loss(256, 1280).cuda()
    #     loss_mfd3 = self.Masked_fd_loss_feature3(S_fpn_output[0],T_fpn_output[0])
    #     loss_mfd4 = self.Masked_fd_loss_feature4(S_fpn_output[1], T_fpn_output[1])
    #     loss_mfd5 = self.Masked_fd_loss_feature5(S_fpn_output[2], T_fpn_output[2])
    #     loss_mfd = (loss_mfd3 + loss_mfd4 + loss_mfd5)/3;
    ...
    def bboxes_diou(self,bboxes_a, bboxes_b, eps=1e-7):
        ...
        c2 = cw ** 2 + ch ** 2 + eps
        rho2 = (bboxes_a[:,None,0] - bboxes_b[:,0]) ** 2 + (bboxes_a[:,None,1]-bboxes_b[:,1]) ** 2  # (cx1-cx2)^2 + (cy1-cy2)^2
        diou = (iou - rho2 / c2).clamp(min=-1,max=1)
