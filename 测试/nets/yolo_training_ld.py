...
from feature_imitation.Attention_Transfer import Attention as At_loss
input_shape = [320, 320]
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, inputs,teacher_inputs,labels):
        ...
        return self.get_losses(x_shifts, y_shifts, expanded_strides, labels, torch.cat(outputs, 1),
                               torch.cat(regs_prob, 1),torch.cat(regs_prob_pre_softmax,1),torch.cat(teacher_regs_prob_pre_softmax,1))
    ...
    def get_ld_loss(self,regs_prob_pre_softmax,teacher_regs_prob_pre_softmax,VLR_masks,T = 10 ,reg_max = 14):
        """
        定位蒸馏的损失:
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs, regs_prob, regs_prob_pre_softmax,teacher_regs_prob_pre_softmax,reg_max=14):
        """
        输入:
        1、x_shifts, y_shifts,expanded_strides 三张特征图上所有网格点的x,y坐标以及对应的stride ，list: [[w1*h1],[w2*h2],[w3*h3]]
        ...
        ################################################################################################################
        # AT_loss
        # loss_At = self.At_loss(S_fpn_output,T_fpn_output)

        ################################################################################################################
        # feature separate loss
