...
from feature_imitation.Prediction_guided_feature_imitation import Prediction_guided_feature_imitation_loss
from feature_imitation.Masked_feature import Masked_FeatureLoss
input_shape = [320, 320]
...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target,eps = 1e-16):
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, inputs,teacher_inputs,labels,T_fpn_output, S_fpn_output,T_backbone_output,S_backbone_output):
        outputs = []
        x_shifts = []
        y_shifts = []
        expanded_strides = []
        regs_prob = []
        teacher_regs_prob_pre_softmax = []
        regs_prob_pre_softmax = []
        teacher_confidence_all = []
        tea_cls_pres = []
        stu_cls_pres = []
        ...
        for k, (stride, output,teacher_output) in enumerate(zip(self.strides, inputs,teacher_inputs)):
            # 共在三个不同大小的特征图上进行目标检测
            # student_reg_out_pre_softmax,teacher_reg_out_pre_softmax:用于ld distillation
            # teacher_confidence : 用于Confidence imitation
            #  tea_cls_pre,stu_cls_pre: 用于Prediction-guided Feature Imitation
            output, grid, reg_prob, \
            student_reg_out_pre_softmax,teacher_reg_out_pre_softmax,\
            teacher_confidence, \
            tea_cls_pre,stu_cls_pre = self.get_output_and_grid(output, teacher_output, k, stride)
            ...
            # 教师网络只需要输出的概率分布,且是softmax之前的结果
            regs_prob_pre_softmax.append(student_reg_out_pre_softmax)
            teacher_regs_prob_pre_softmax.append(teacher_reg_out_pre_softmax)

            # 置信度蒸馏 ################################################################################
            teacher_confidence_all.append(teacher_confidence)

            # 类别预测引导的特征模仿
            tea_cls_pres.append(tea_cls_pre)
            stu_cls_pres.append(stu_cls_pre)



        # torch.cat(outputs, 1),torch.cat(regs_prob, 1),此处将列表中三个张量在维度1进行拼接，因此 结果为:[b,w1*h1+w2*h2+w3*h3,6] 和 [b,w1*h1+w2*h2+w3*h3,68]
        # torch.cat(teacher_confidence_all,1): 获得教师置信度预测的特征图
        return self.get_losses(x_shifts, y_shifts, expanded_strides, labels, torch.cat(outputs, 1),
                               torch.cat(regs_prob, 1),torch.cat(regs_prob_pre_softmax,1),
                               torch.cat(teacher_regs_prob_pre_softmax,1),torch.cat(teacher_confidence_all,1),
                               tea_cls_pres,stu_cls_pres,T_fpn_output, S_fpn_output,T_backbone_output,S_backbone_output)

    def get_output_and_grid(self, output,teacher_output, k, stride, reg_max=14):
        """
        输入:
        ...
        student_reg_out_pre_softmax = output[..., : (reg_max + 1)*4]  # 获得未softmax前的回归值 [b,w*h,(reg_max + 1)*4]

        teacher_output = teacher_output.flatten(start_dim=2).permute(0,2,1)
        tercher_confidence = teacher_output[...,(reg_max + 1) * 4]  # 获得 教师网络输出置信度特征头
        # print(tercher_confidence.shape)
        teacher_reg_out_pre_softmax = teacher_output[...,:(reg_max + 1) * 4] # 获得未经过softmax的回归值 [b,w*h,(reg_max + 1)*4]

        # 预测引导的特征模仿知识蒸馏,这里保存分类的预测结果作为特征模仿的引导 #####################################################
        teacher_cls_predition = torch.sigmoid(teacher_output[..., (reg_max + 1) * 4 + 1:])  # 教师的类别预测 [b,w*h,2]
        student_cls_predition = torch.sigmoid(output[..., (reg_max + 1) * 4 + 1:])  # 学生的类别预测特征图 [b,w*h,2]
        ################################################################################################################
        ...
        # teacher_reg_output,teacher_reg_out_pre_jifen = self.Integral(teacher_output[...,:(reg_max + 1) * 4])
        output = torch.cat(
            [reg_output, output[..., (reg_max + 1) * 4:(reg_max + 1) * 4 + 1], output[..., (reg_max + 1) * 4 + 1:]], 2)
        # teacher_output = torch.cat(
        #     [teacher_reg_output, teacher_output[..., (reg_max + 1) * 4:(reg_max + 1) * 4 + 1], teacher_output[..., (reg_max + 1) * 4 + 1:]], 2)
        ...
        return output, grid, reg_out_pre_jifen,student_reg_out_pre_softmax,teacher_reg_out_pre_softmax,tercher_confidence,teacher_cls_predition,student_cls_predition

    # ld distillation start ############################################################################################
    def get_ld_loss(self,regs_prob_pre_softmax,teacher_regs_prob_pre_softmax,VLR_masks,T = 15 ,reg_max = 14):
        """
        定位蒸馏的损失:
        regs_prob: shape——[b,all_anchor_points,4 * (regmax+1)] 学生网络的定位的概率分布
        teacher_regs_prob: shape——[b,all_anchor_points,4*(regmax+1)] 教师网络的定位概率分布
        VLR_masks:[total_num_anchors * batch_size]
        """
        Students_VLR_and_Main = (regs_prob_pre_softmax.reshape(-1, 4, reg_max + 1)[VLR_masks]).reshape(-1,
                                                                                                       reg_max + 1)  # [b * all_anchor_points,4,reg_max]
        Teachers_VLR_and_Main = (teacher_regs_prob_pre_softmax.reshape(-1, 4, reg_max + 1)[VLR_masks]).reshape(-1,
                                                                                                               reg_max + 1)
        n = VLR_masks.sum() * 4

        distillation_loss = F.kl_div(F.log_softmax(Students_VLR_and_Main / T, dim=1),
                                     F.softmax(Teachers_VLR_and_Main / T, dim=1), reduction='sum') * (T * T)  

        return distillation_loss / n
    
    def get_ld_loss_JS_Div(self, regs_prob_pre_softmax, teacher_regs_prob_pre_softmax, VLR_masks, T=15, reg_max=14):
        """
        定位蒸馏的损失:
        regs_prob: shape——[b,all_anchor_points,4 * (regmax+1)] 学生网络的定位的概率分布
        teacher_regs_prob: shape——[b,all_anchor_points,4*(regmax+1)] 教师网络的定位概率分布
        VLR_masks:[total_num_anchors * batch_size]
        """
        Students_VLR_and_Main = (regs_prob_pre_softmax.reshape(-1, 4, reg_max + 1)[VLR_masks]).reshape(-1,reg_max + 1)  # [b * all_anchor_points,4,reg_max]
        Teachers_VLR_and_Main = (teacher_regs_prob_pre_softmax.reshape(-1, 4, reg_max + 1)[VLR_masks]).reshape(-1,reg_max + 1)
        n = VLR_masks.sum()
        p_output = F.softmax(Teachers_VLR_and_Main / T, dim=1)
        q_output = F.softmax(Students_VLR_and_Main / T, dim=1)
        log_mean_output = ((p_output + q_output) / 2).log()
        disillaiton_loss1 = F.kl_div(log_mean_output,
                                 p_output, reduction='none').sum(1) * (T * T)
        disillaiton_loss2 = F.kl_div(log_mean_output,
                                q_output, reduction='none').sum(1) * (T * T)

        distillation_loss = (disillaiton_loss1.sum() + disillaiton_loss2.sum()) / 2

        return distillation_loss / n
    

    # ld 中的 VLR mask
    def get_VLR_mask(self, labels, outputs,p_pos=0.5, p_vlr=0.6):
        """
        获得VLR掩膜
        1、计算每个gt和预测框的DIOU
        2、正样本的DIOU阈值: 0.5
        3、VLR的DIOU阈值: 0.25 * 0.5

        输入参数:
        1、x_shifts, y_shifts, expanded_strides:三张特征图上所有网格点的x,y坐标以及对应的stride ，list: [[w1*h1],[w2*h2],[w3*h3]]
        2、labels: 一个批次中所有图片中的ground_truth标签
        3、outputs: shape[b,all_anchor_points,c],格式:[cx,cy,w,h]
        4、p_pos=0.5 正样本的DIOU阈值
        5、p_vlr=0.25 VLR的参数
        """
        VLR_masks = []
        ...
        for batch_idx in range(bbox_preds.shape[0]): # 遍历一个批次中所有图片的结果
            num_gt = len(labels[batch_idx])  # 每幅图的gt个数
            if(num_gt == 0):
                is_in_VLR_and_main_all = outputs.new_zeros(total_num_anchors).bool()

            else:
                gt_bboxes_per_image = labels[batch_idx][..., :4]  # [num_gt,4] 每张图片的所有gt对应的四个回归值[cx,cy,w,h]
                bboxes_preds_per_image = bbox_preds[batch_idx]  # bboxes_preds_per_image: [all_pred_bboxes, [cx, cy, w, h]] 每张图片所有预测框对应的四个回归值 [cx,cy,w,h]
                pair_wise_ious = self.bboxes_diou(gt_bboxes_per_image, bboxes_preds_per_image, False)  # [num_gt,all_anchor_points],即计算预测框和所有gt的IOU
                is_in_VLR_and_main = torch.gt(pair_wise_ious, p_pos * p_vlr)  # [num_gt, total_num_anchors] 在预测框和所有gt的IOU矩阵中挑选出比p_pos * p_vlr大的位置
                is_in_VLR_and_main_all = is_in_VLR_and_main.sum(dim=0) > 0  # [total_num_anchors] 按维度0求和，然后根据和是否>0来判断该预测框是否与某个gt的IOU>p_pos * p_vlr

            VLR_masks.append(is_in_VLR_and_main_all)

        return torch.cat(VLR_masks, 0)  # [total_num_anchors * batch_size] 8400

    # ld distillation end ##############################################################################################

    # Confidence imitation distillation start ##########################################################################
    def get_confidence_feature_imitation(self,student_confidence_all,teacher_confidence_all):
        MSE_loss = nn.SmoothL1Loss(reduction='mean') # reduce=True: 返回标量形式的loss; reduce=False: 返回标量形式的loss
        return MSE_loss(student_confidence_all.reshape(-1,1),teacher_confidence_all.reshape(-1,1))
    # Confidence imitation distillation end ############################################################################

    # Prediction-guided feature imitation start ########################################################################
    def get_Prediction_guided_feature_imitation_loss(self,tea_cls_pres,stu_cls_pres,T_fpn_output,S_fpn_output):

        device = stu_cls_pres[0].device
        self.Pre_guided_imitation_loss3 = Prediction_guided_feature_imitation_loss(64, 128).to(device)

        self.Pre_guided_imitation_loss4 = Prediction_guided_feature_imitation_loss(128, 256).to(device)

        self.Pre_guided_imitation_loss5 = Prediction_guided_feature_imitation_loss(256, 512).to(device)

        
        loss_PGF3 = self.Pre_guided_imitation_loss3(tea_cls_pres[0],stu_cls_pres[0],T_fpn_output[0],S_fpn_output[0]).to(device)
        loss_PGF4 = self.Pre_guided_imitation_loss4(tea_cls_pres[1],stu_cls_pres[1],T_fpn_output[1],S_fpn_output[1]).to(device)
        loss_PGF5 = self.Pre_guided_imitation_loss5(tea_cls_pres[2],stu_cls_pres[2],T_fpn_output[2],S_fpn_output[2]).to(device)
        loss_PGF = ((loss_PGF3 + loss_PGF4 + loss_PGF5)/3).to(device);
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs, regs_prob,
                   regs_prob_pre_softmax,teacher_regs_prob_pre_softmax,teacher_confidence_all,tea_cls_pres,stu_cls_pres,T_fpn_output, S_fpn_output,T_backbone_output,S_backbone_output,reg_max=14):
        """
        输入:
        1、x_shifts, y_shifts,expanded_strides 三张特征图上所有网格点的x,y坐标以及对应的stride ，list: [[w1*h1],[w2*h2],[w3*h3]]
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                # ld ： 前背景分离mask ###################################
                VLR_mask = outputs.new_zeros(total_num_anchors).bool()
                back_ground_mask = ~ VLR_mask
                #######################################################
                ...
                # 进行标签分配
                gt_matched_classes, fg_mask, pred_ious_this_matching, matched_gt_inds, num_fg_img,VLR_mask = self.get_assignments(
            ...
            cls_targets.append(
                cls_target)  # [[num_fg1,classes_num],[num_fg2,classes_num],[num_fg3,classes_num],[num_fg4,classes_num]]
            reg_targets.append(reg_target)

            obj_targets.append(obj_target.type(cls_target.type()))
            fg_masks.append(fg_mask)
            VLR_masks.append(VLR_mask)  # 将每一张图的VLR_mask都加入list
            back_ground_masks.append(back_ground_mask)
        ...
        DFL_targets = DFL_targets.to(torch.float64) / DFL_targets_strides
        ...
        ###############################################################################################################
        # ld 分离前背景 + js_div
        VLR = torch.cat(VLR_masks, 0) # list 转 tensor : [total_num_anchors * batch_size]
        # back_ground = torch.cat(back_ground_masks,0)
        loss_distillation1 = self.get_ld_loss_JS_Div(regs_prob_pre_softmax, teacher_regs_prob_pre_softmax, VLR) # 蒸馏前景
        # loss_distillation2 = self.get_ld_loss_JS_Div(regs_prob_pre_softmax, teacher_regs_prob_pre_softmax, back_ground) # 蒸馏背景
        loss_distillation =5 * loss_distillation1
        print(loss_distillation)
        VLR_masks = self.get_VLR_mask(labels, outputs)  # [total_num_anchors * batch_size]
        loss_distillation = self.get_ld_loss(regs_prob_pre_softmax, teacher_regs_prob_pre_softmax, VLR_masks)
        print(5 * loss_distillation)
        ################################################################################################################

        ################################################################################################################
        # confidence imitation
        loss_mse = self.get_confidence_feature_imitation(obj_preds,teacher_confidence_all)

        ################################################################################################################

        ################################################################################################################
        loss_PGF_fpn = self.get_Prediction_guided_feature_imitation_loss(tea_cls_pres, stu_cls_pres,T_fpn_output, S_fpn_output)
        loss_PGF_backbone = self.get_Prediction_guided_feature_imitation_loss(tea_cls_pres, stu_cls_pres,T_backbone_output,S_backbone_output)
        loss_PGF = (loss_PGF_fpn + loss_PGF_backbone) / 2
        ################################################################################################################
        ...
        # reg_weight = 6
        reg_weight = 5.5
        # loss = reg_weight * loss_iou + loss_obj + loss_cls +  + loss_distillation
        #  + 10 * sum(loss_At)
        loss = reg_weight * loss_iou +  loss_obj + loss_cls + 0.25 * loss_dfl
        
        # return loss / num_fg  + 5 * loss_distillation
#         return loss / num_fg  + 160000 * loss_PGF
        return loss / num_fg  + 100000 * loss_PGF + 5 * loss_distillation  + 0.5 * loss_mse
    ...
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image,
        ...
        VLR_mask,_ = self.get_in_boxes_info(gt_bboxes_per_image, expanded_strides, x_shifts,
                                                                 y_shifts, total_num_anchors, num_gt,center_radius=4)
        # -------------------------------------------------------#
        #   fg_mask                 [total_num_anchors]
        ...
        bboxes_preds_per_image = bboxes_preds_per_image[fg_mask]  # [候选正样本数，4]
        ...
        return gt_matched_classes, fg_mask, pred_ious_this_matching, matched_gt_inds, num_fg,VLR_mask

    def bboxes_diou(self,bboxes_a, bboxes_b, eps=1e-7):
        """
        bboxes_a : [num_gt,4]  cx,cy,w,h
        bboxes_b : [num_yuce,4] cx,cy,w,h
        """
        # cx,cy,w,h -> xmin,ymin -> 交集的左上角坐标 iou_xmin,iou_ymin  即shape:[num_gt,num_yuce,2]
        # 计算交集的坐标 tlbr
        tl = torch.max(
            (bboxes_a[:, None, :2] - bboxes_a[:, None, :2] / 2),  # [num_gt,1,2] : None,在此添加一个大小为1的维度
            (bboxes_b[:, :2] - bboxes_b[:, 2:] / 2), # [num_pred,2]
            # 根据广播机制,[num_gt,1,2]会先变成[num_gt,num_pred,2]
            # 然后 [num_pred,2] 会变成 [num_gt,num_pred,2]
            # 因此 tl最后的shape为 [num_gt,num_pred,2],表示每个gt和所有预测框取tl
        )

        # cx,cy,w,h -> xmax,ymax  -> 交集的右下角坐标 iou_xmax,iou_max
        br = torch.min(
            (bboxes_a[:, None, :2] + bboxes_a[:, None, 2:] / 2),
            (bboxes_b[:, :2] + bboxes_b[:, 2:] / 2),
        )

        area_a = torch.prod(bboxes_a[:, 2:], 1) # prod：获得张量给定维度上的乘积 , gt面积
        area_b = torch.prod(bboxes_b[:, 2:], 1) # pred 面积
        en = (tl < br).type(tl.type()).prod(dim=2) # (tl < br).type(tl.type()).prod(dim=2) :  t<b,l<r时才有交集
        # (tl < br).type(tl.type()): 将所有满足t<b或l<r的值置为1，否则为0,.prod(dim=2): 再筛选出t<b且l<r的结果，综合筛选出 与gt有交集的预测框， [num_gt,num_pred]
        area_i = torch.prod(br - tl, 2) * en # 计算所有预测框和gt的交集的面积，如果没有交集，就为0
        iou = area_i / (area_a[:, None] + area_b - area_i) # 计算gt和所有预测框的iou

        # 计算能包含gt和pred的最小方框的对角线距离
        tl_bibao = torch.min(
            (bboxes_a[:, None, :2] - bboxes_a[:, None, 2:] / 2),
            (bboxes_b[:, :2] - bboxes_b[:, 2:] / 2),
        )
        ...
        cw = br_bibao[...,0] - tl_bibao[...,0]
        ch = br_bibao[...,1] - tl_bibao[...,1]
        c2 = cw ** 2 + ch ** 2 + eps  # 对角线的平方

        rho2 = (bboxes_a[:,None,0] - bboxes_b[:,0]) ** 2 + (bboxes_a[:,None,1]-bboxes_b[:,1]) ** 2  # (cx1-cx2)^2 + (cy1-cy2)^2 即 gt 和 pred的中心点距离
        # DIoU : iou - center(gt,pred)^2 / c^2
        diou = (iou - rho2 / c2).clamp(min=-1,max=1) # [num_gt,num_pred] : gt和所有预测框的diou
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        topk_ious, _ = torch.topk(pair_wise_ious, n_candidate_k,
                                  dim=1)  # 为每个gt找n_candidate_k个IOU最大的点  [num_gt,n_candidate_k]
        dynamic_ks = torch.clamp(topk_ious.sum(1).int(),
                                 min=1)  # 然后把这10个IOU求和，并确保IOU最小为1 [num_gt,1] 每一行表示要为这个gt分配的正样本个数
        ...
        for gt_idx in range(num_gt):
            ...
            _, pos_idx = torch.topk(cost[gt_idx], k=dynamic_ks[gt_idx].item(),
                                    largest=False)  # cost: [num_gt, fg_mask]  从fg_mask个候选样本中挑出k个cost最小的点
